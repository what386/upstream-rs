...
Windows installations require the latest supported Microsoft Visual C++ v14 Redistributable, matching the system architecture. Install the package from [Microsoft](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist) before running the bootstrap script. The Windows installer checks for the runtime and stops with an actionable error before downloading Upstream when it is missing.
