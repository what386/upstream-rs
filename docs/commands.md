...
upstream probe <repo-or-url> [name] [-p <provider>] [--channel <channel>] [--limit <n>] [--verbose] [--include-incompatible]
...
- `probe` shows releases and compatible assets, opens an interactive asset picker, prompts for the package name with an inferred default when `[name]` is omitted, and installs the selected asset. Use `--include-incompatible` to show all release assets, or `--dry-run` / `--json` to inspect without installing.
